...
function initializePage() {
  ...
  /*
  $('#search_reps').click(()=> { //#search_reps

    // get the address from the search bar
    const address = $('#searchButton').val(); //#search
...
// press enter to trigger search button click
function locationCall(pollURL, ev) {
  console.log(ev.result.place_name);
	// make the API call with the Address
	$.ajax({
		type: 'GET',
		url: 'https://www.googleapis.com/civicinfo/v2/representatives?key=AIzaSyBrvfLVDyTJ1Ar8EdM1aZda_9141qQcCh4&address=' + ev.result.place_name,
		dataType: 'json',
		success: function(data) {
			console.log(data);
